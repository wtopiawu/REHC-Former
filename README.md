...
[中文说明](README_zh.md) · [Segmentation](segmentation/README.md) · [Data format](docs/DATA_FORMAT.md)
...
| Path | Purpose |
|---|---|
| `rehc_former/` | Pose models, ablations, data loading, losses, and training utilities |
...
Datasets and pretrained weights are not currently available for download. Prepare your data and train the required models before inference. An existing RGB-aligned mask must encode foreground as 255 and background as 0:
...
The JSON contains `T_EC`, its inverse `T_CE`, translation units and quaternion order (`xyzw`). The command needs no ground-truth pose, robot observations or calibration target.
...
Tests check model and pipeline behavior using synthetic data. See [implementation notes](docs/PAPER_ALIGNMENT.md) and [validation details](docs/VALIDATION.md) for experiment configuration and validation scope.
